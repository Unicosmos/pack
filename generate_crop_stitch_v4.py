...
核心功能：
1. 透视变换：对原图和粘贴图都进行透视变换
2. 高密度筛选：优先选择目标数量多的图片作为原图
3. 随机区域亮度调整
4. 噪声增强
5. 左右/上下对齐拼接

使用方法：
    python generate_crop_stitch_v4.py --input pack_original --output mypack_crop --num-images 500
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description='随机裁剪拼接数据集生成脚本 V4')
    ...
    parser.add_argument('--brightness-prob', type=float, default=0.5,
                       help='随机亮度调整概率 (默认: 0.5)')
...
def apply_perspective_transform(image, boxes, angle_range=60):
    h, w = image.shape[:2]
    ...
    if transform_type == 'top_narrow':
        shift = random.uniform(0.15, 0.35) * w
        ...
        shift = random.uniform(0.15, 0.35) * h
...
def apply_random_brightness(image, num_regions=2):
    h, w = image.shape[:2]
    result = image.copy()
    ...
    for _ in range(num_regions):
        ...
        result[y:y+region_h, x:x+region_w] = np.clip(
            result[y:y+region_h, x:x+region_w] * brightness_factor, 0, 255
        ).astype(np.uint8)
...
def generate_image(data, args):
    ...
    if random.random() < args.perspective_prob:
        base_img, base_boxes, _ = apply_perspective_transform(base_img, base_boxes)
    
    # 对粘贴图也进行透视变换
    if random.random() < args.perspective_prob:
        crop_img, crop_boxes, _ = apply_perspective_transform(crop_img, crop_boxes)
    
    direction = random.choice(['horizontal', 'vertical'])
    
    if direction == 'horizontal':
        result, new_boxes, new_w, new_h = horizontal_stitch(
            base_img, base_boxes, crop_img, crop_boxes,
            args.max_occlusion, args.max_expand
        )
    else:
    ...
    # 随机区域亮度调整
    if random.random() < args.brightness_prob:
        result = apply_random_brightness(result, num_regions=random.randint(1, 3))
