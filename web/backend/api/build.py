...
SKU_OUTPUT_DIR = BASE_DIR / "training" / "sku" / "sku_output"
SKU_LIBRARY_DIR = BASE_DIR / "data" / "sku_library"
BUILD_SCRIPT = BASE_DIR / "training" / "sku" / "build_sku_library.py"
EXTRACT_SCRIPT = BASE_DIR / "training" / "sku" / "extract_features.py"
STATUS_FILE = BASE_DIR / "data" / "build_status.json"
...
def run_build():
    ...
    try:
        cmd = [
            "python",
            str(BUILD_SCRIPT),
            "--input", str(SKU_OUTPUT_DIR),
            "--output", str(SKU_LIBRARY_DIR),
            "--seed", "0"
        ]
        ...
        if result.returncode == 0:
            ...
            meta_path = SKU_LIBRARY_DIR / "meta.json"
            if meta_path.exists():
...
def run_extract_feature():
    ...
    try:
        cmd = [
            "python",
            str(EXTRACT_SCRIPT),
            "--input", str(SKU_LIBRARY_DIR),
            "--csv", "sku_library.csv",
            "--weights", str(BASE_DIR / "data" / "models" / "sku" / "vits16_dino_finetuned.pth"),
            "--batch-size", "16",
            "--device", "cpu"
        ]
        
        # 检查必要的文件是否存在
        if not (SKU_LIBRARY_DIR / "sku_library.csv").exists():
        ...
        if result.returncode == 0:
            ...
            meta_path = SKU_LIBRARY_DIR / "feature_meta.json"
            if meta_path.exists():
                with open(meta_path, "r", encoding="utf-8") as f:
...
def run_build_and_extract():
    ...
    try:
        ...
        feature_meta_path = SKU_LIBRARY_DIR / "feature_meta.json"
...
async def get_info():
    info = {
        ...
        "sku_library": {
            "exists": SKU_LIBRARY_DIR.exists(),
            "path": str(SKU_LIBRARY_DIR),
            "has_images": (SKU_LIBRARY_DIR / "images").exists() if SKU_LIBRARY_DIR.exists() else False,
            "has_csv": (SKU_LIBRARY_DIR / "sku_library.csv").exists() if SKU_LIBRARY_DIR.exists() else False,
            "has_meta": (SKU_LIBRARY_DIR / "meta.json").exists() if SKU_LIBRARY_DIR.exists() else False,
            "has_features": (SKU_LIBRARY_DIR / "sku_features.npy").exists() if SKU_LIBRARY_DIR.exists() else False
        }
    }
